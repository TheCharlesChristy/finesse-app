...
import { Bell, Download, Upload, FileText, FileUp, FileCog, Palette, RefreshCcw, RefreshCw, SlidersHorizontal, Trash2, Link2, FileJson, Plus, Wand2, Zap, Info, HardDrive, ShieldCheck, Lock, Undo2 } from 'lucide-react';
...
/**
 * Settings is eleven cards deep, so it is a consolidated page like Activity and
 * Insights: five tabs, grouped by what a person came here to do rather than by
 * which part of the code owns the card.
 *
 * Backup, storage durability and a full reset all sit under Data together —
 * they are the three things that decide whether the data survives, and having
 * them eight screens below the colour picker was the old layout's real problem.
 */
const TABS = [
...
export default function Settings({
  ...
  const handleAppearanceChange = (patch) => onSaveSettings?.({ appearance: { ...(settings?.appearance ?? {}), ...patch } });
  ...
  return (
    <div className="fade-in stack-lg">
      ...
      {tab === 'appearance' && (
        ...
        <AppearanceSettings appearance={settings?.appearance} theme={resolvedTheme} onChange={handleAppearanceChange} />
        <p className="field-hint">Appearance is saved with this account, so a personal and a business budget can look
          different — which is the quickest way to be sure which one you are looking at.</p>
        </div>
      )}
      ...
      {tab === 'budget' && (
          ...
          {incomes.length === 0 ? (
            ...
            <>
              <div className="mobile-row-stack" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                <select className="input" aria-label="Income to reset" value={selectedIncomeId} onChange={e => { setResetIncomeId(e.target.value); setResetStatus(''); }}
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            <Wand2 size={16} color="var(--accent)" aria-hidden="true" />
            ...
            <>
              ...
              <div className="mobile-row-stack" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ flex: '2 1 160px' }}>
                  ...
                  <input id="rule-match" className="input" placeholder="e.g. Tesco" value={ruleMatch}
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            <Zap size={16} color="var(--accent-4)" aria-hidden="true" />
            ...
            <>
              ...
              <div className="mobile-row-stack" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ flex: '2 1 140px' }}>
                  ...
                  <input id="template-name" className="input" placeholder="e.g. Coffee" value={templateName}
                    onChange={e => setTemplateName(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && handleAddTemplate()} />
                </div>
                <div style={{ flex: '1 1 100px' }}>
                  <label htmlFor="template-amount" className="field-label">Amount (£)</label>
                  <input id="template-amount" className="input" type="number" min="0" step="0.01" placeholder="3.20"
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            <Bell size={16} color="var(--accent-2)" aria-hidden="true" />
      ...
      {tab === 'privacy' && (
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            <Lock size={16} color="var(--accent-3)" aria-hidden="true" />
          ...
          {!lockSupported() ? (
            <div style={{ color: 'var(--text-muted)', fontSize: 12, borderTop: '1px solid color-mix(in srgb, var(--text-primary) 8%, transparent)', paddingTop: 16 }}>
            ...
            <div style={{ borderTop: '1px solid color-mix(in srgb, var(--text-primary) 8%, transparent)', paddingTop: 16 }}>
              ...
              <div className="field" style={{ marginBottom: 14 }}>
                ...
                <select id="lock-delay" className="input"
      ...
      {tab === 'data' && (
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            <HardDrive size={16} color="var(--accent-3)" aria-hidden="true" />
          ...
          {estimate && (
            <div style={{ borderTop: '1px solid color-mix(in srgb, var(--text-primary) 8%, transparent)', marginTop: 16, paddingTop: 16 }}>
              ...
              <div style={{ height: 6, borderRadius: 'var(--radius-xs)', background: 'color-mix(in srgb, var(--text-primary) 7%, transparent)', overflow: 'hidden' }}>
                <div style={{
                  width: `${Math.max(1, Math.min(100, estimate.percent))}%`, height: '100%',
                  background: estimate.percent > 85 ? 'var(--danger)' : 'var(--accent-3)',
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          ...
          <div style={{ borderTop: '1px solid color-mix(in srgb, var(--text-primary) 8%, transparent)', marginTop: 18, paddingTop: 18 }}>
            ...
            {stagedBudgetConfig ? (
              <div style={{
                background: 'color-mix(in srgb, var(--accent-2) 8%, transparent)', borderRadius: 'var(--radius-md)', padding: '12px 14px',
            ...
            {budgetConfigUndo?.appliedAt && (
              <div style={{
                background: 'color-mix(in srgb, var(--accent) 8%, transparent)', borderRadius: 'var(--radius-md)', padding: '12px 14px',
                marginBottom: 12, fontSize: 12, lineHeight: 1.5,
              }}>
                <div style={{ fontWeight: 600, marginBottom: 3 }}>
      ...
      {tab === 'about' && (
        ...
        <div className="card mobile-card-pad" style={{ borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            <Info size={16} color="var(--accent-2)" aria-hidden="true" />
